...
# vital = read.csv('data/Adam_Vital_Simple.csv')
...
treatment_sub = treatment %>%
  mutate(tx_cat = names(treatment[,5:18])[max.col(treatment[,5:18])]) %>%
  filter(redcap_repeat_instrument!='') %>%
  mutate(maintenance_consolidation = ifelse(rx_indication %in% c(3,4,6,7,8), 1, 0)) %>%
  select(subject_id, redcap_repeat_instrument, tx_cat, maintenance_consolidation, date_treatment) %>%
...
event_data = response_sub %>%
  ...
  mutate(
    ...
    treatment_point = ifelse(str_detect(event_type, 'treatment'), 1, 0),
...
- **Consolidation or maintenance**: 
  - 1 : when rx_indication in [3 (Consolidation of CR),4(Maintenance of CR),6(Induction f/b "consolidation" (no restaging between)),7(Induction f/b "maintenance" (no restaging between)),8 (Consolidation f/b "maintenance" (no restaging between))];
  - 0 otherwise.
...
event_sum = event_data %>%
  ...
  summarise(is_dead = ifelse('Dead' %in% redcap_repeat_instrument, 'Dead', 'Alive'),
            ...
            n_treatment = sum(redcap_repeat_instrument == 'treatment_event'),
...
Patient counts per each number of treatment, e.g., 56 patients has 0 treatments.
...
event_sum %>% ggplot(aes(x = n_treatment, fill = is_treatment)) + geom_histogram() + scale_fill_discrete('') + theme(legend.position = 'bottom') + scale_x_continuous('Number of treatments', breaks = seq(0, 17)) + scale_y_continuous('Number of patients', breaks = seq(0, 150, by = 30))
```
...
```{r}
event_data %>% select(-relapse_point, -treatment_point) %>% subset(subject_id == '2236847') %>% arrange(event_date) %>% datatable(filter = 'top') 
```

## Duration between decision points and following relapse

```{r}
relapse_dura_since_last_decision %>% select(-relapse_point, -treatment_point, -decision_point, -post_treatment, -death_date, -vital_status, -maintenance_consolidation) %>% subset(subject_id == '2236847') %>% arrange(event_date) %>% datatable(filter = 'top') 
```

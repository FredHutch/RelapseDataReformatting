...
treatment_sub = treatment %>%
  ...
  mutate(maintenance_consolidation = ifelse(rx_indication %in% c(2,3,6,7,8), 1, 0)) %>%
...
- **Consolidation or maintenance**: 1 when rx_indication in [2,3,6,7,8], 0 otherwise.
...
event_sum %>% ggplot(aes(x = n_treatment, fill = is_treatment)) + geom_histogram() + scale_fill_discrete('') + theme(legend.position = 'bottom') + scale_x_continuous('Number of treatments', breaks = seq(0, 17)) + ylab('Number of patients')
```
...
event_sum %>% ggplot(aes(x = n_decision)) + geom_histogram() + scale_x_continuous('Number of decision points', breaks = seq(0, 17)) + ylab('Number of patients') + ggtitle('Number of decision points per patient')
```
